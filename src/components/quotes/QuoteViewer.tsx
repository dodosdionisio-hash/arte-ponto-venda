    ...
    <div className="space-y-6">
      ...
      <div className="print-container bg-gradient-to-br from-white to-gray-50 text-black p-10 rounded-xl shadow-xl print:p-0 print:shadow-none print:bg-white">
        ...
        <div className="bg-primary/5 rounded-lg p-6 mb-6">
          <h2 className="text-2xl font-bold text-primary mb-4">ORÇAMENTO #{quote.quote_number}</h2>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div className="space-y-2">
              <h3 className="font-semibold text-gray-700 mb-2">Dados do Cliente</h3>
              <p className="text-sm"><span className="font-medium">Nome:</span> {quote.customers?.name}</p>
              {quote.customers?.cpf_cnpj && <p className="text-sm"><span className="font-medium">CPF/CNPJ:</span> {quote.customers.cpf_cnpj}</p>}
              {quote.customers?.phone && <p className="text-sm"><span className="font-medium">Telefone:</span> {quote.customers.phone}</p>}
              {quote.customers?.email && <p className="text-sm"><span className="font-medium">E-mail:</span> {quote.customers.email}</p>}
            </div>
            <div className="space-y-2">
              <h3 className="font-semibold text-gray-700 mb-2">Informações do Orçamento</h3>
              <p className="text-sm"><span className="font-medium">Data de Emissão:</span> {new Date(quote.issue_date).toLocaleDateString('pt-BR')}</p>
              <p className="text-sm"><span className="font-medium">Válido até:</span> {new Date(quote.valid_until).toLocaleDateString('pt-BR')}</p>
            </div>
          </div>
        </div>

        <div className="mb-6 overflow-hidden print:overflow-visible rounded-lg border-2 border-gray-200">
        ...
        <div className="flex justify-end mb-6">
          <div className="bg-primary text-primary-foreground rounded-lg p-6 shadow-lg">
            ...
            <p className="text-3xl font-bold">R$ {Number(quote.total_amount).toFixed(2)}</p>
          </div>
        </div>
